...
BALANCED_SUBSET_CATEGORY_COUNTS = {category: 50 for category in TASK_PAIRS}
ALLOWED_CATEGORY_COUNTS = (
    Counter(EXPECTED_CATEGORY_COUNTS),
    Counter(BALANCED_SUBSET_CATEGORY_COUNTS),
)
...
def bbox_from_reference_space(
    bbox: list[float], reference: tuple[float, float, list[float]], image: Image.Image
) -> list[float]:
    """Map any bbox expressed in the question's declared image space."""
    reference_width, reference_height, _ = reference
    return [
        bbox[0] * image.width / reference_width,
        bbox[1] * image.height / reference_height,
        bbox[2] * image.width / reference_width,
        bbox[3] * image.height / reference_height,
    ]
...
GLOBAL_PLUS_CROP_STAGE2_INSTRUCTION = """
...
def make_concise_reference_stage2_prompt(
    stage1_prompt: str, crop_count: int
) -> str:
    """Keep both Stage 2 images in the user turn with global coordinates."""
    if ZOOMEARTH_INSTRUCTION not in stage1_prompt:
        raise ValueError("stage1 prompt does not contain the ZoomEarth instruction")
    concise_prompt = stage1_prompt.replace(
        ZOOMEARTH_INSTRUCTION, GLOBAL_PLUS_CROP_STAGE2_INSTRUCTION, 1
    )
    assistant_suffix = "<|im_end|><|im_start|>assistant\n"
    if not concise_prompt.endswith(assistant_suffix):
        raise ValueError("stage1 prompt does not end with the assistant generation prefix")
    user_prompt = concise_prompt[: -len(assistant_suffix)]
    return user_prompt + IMAGE_TOKEN * crop_count + assistant_suffix


REFERENCE_STAGE2_INSTRUCTION = """
The image is a high-resolution crop of the original satellite image. The image
resolution and reference bounding box in the question are expressed in this
crop's local coordinates. Analyze the object inside that exact bounding box and
answer the original multiple-choice question. Do not request another crop.
Return exactly one <answer>...</answer> block and no text outside that block.
"""
...
def enable_segmented_vision_attention() -> None:
    ...
    attention_class = getattr(
        modeling_qwen2_5_vl, "Qwen2_5_VLVisionSdpaAttention", None
    )
    if attention_class is not None:
        attention_class.forward = segmented_vision_attention_forward
    # Qwen3.5 and current Qwen2.5 already process cu_seqlens segments natively.
...
def run(args: argparse.Namespace) -> None:
    ...
    if observed_counts not in ALLOWED_CATEGORY_COUNTS:
        raise RuntimeError(
    ...
    with args.output.open(mode, encoding="utf-8", buffering=1) as output_handle:
        for position in tqdm(positions, desc="Evaluating XLRS-lite"):
            ...
            try:
                ...
                stage1_question = doc["question"]
                stage1_reference = extract_reference_bbox(stage1_question)
                ...
                raw_bbox_valid = (
                    model_bbox_raw is not None
                    and len(model_bbox_raw) == 4
                    and model_bbox_raw[2] > model_bbox_raw[0]
                    and model_bbox_raw[3] > model_bbox_raw[1]
                )
                try:
                    if not raw_bbox_valid:
                        raise ValueError("invalid model bbox")
                    if stage1_reference is not None:
                        mapped_bbox = bbox_from_reference_space(
                            model_bbox_raw, stage1_reference, original_images[0]
                        )
                        original_bbox = clip_bbox_to_image(
                            mapped_bbox, original_images[0]
                        )
                        original_bboxes = [original_bbox]
                        bbox_resized = clip_bbox_to_image(
                            map_bbox_between_images(
                                original_bbox, original_images[0], global_images[0]
                            ),
                            global_images[0],
                        )
                    else:
                ...
                if bbox_resized is not None:
                    ...
                    stage2_prompt_style = "concise_global_plus_crop"
