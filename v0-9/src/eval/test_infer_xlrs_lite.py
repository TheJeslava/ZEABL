...
from infer_xlrs_lite import (
    ...
    make_concise_reference_stage2_prompt,
    make_stage1_prompt,
    make_stage2_prompt,
...
class InferXLRSAdapterTests(unittest.TestCase):
    ...
    def test_segmented_vision_attention_matches_block_diagonal_attention(self):
        ...
        torch.manual_seed(7)
        attention = modeling_qwen2_5_vl.Qwen2_5_VLVisionSdpaAttention(
            dim=32, num_heads=4
        ).eval()
    ...
    def test_reference_stage2_prompt_omits_stage1_think(self):
        sample = self.sample(self.FOUR_CHOICE_CATEGORY)
        ...
        prompt = make_stage2_prompt(
            stage1_prompt, stage1_output, 1, omit_stage1_think=True
        )

        self.assertEqual(prompt.count(IMAGE_TOKEN), 2)
        self.assertTrue(prompt.startswith(stage1_prompt))
        self.assertIn("question with reference coordinates", prompt)
        self.assertNotIn("<think>locate target</think>", prompt)
        self.assertIn('"bbox_2d": [1, 2, 3, 4]', prompt)
        self.assertNotIn("<answer>A</answer>", prompt)
        self.assertIn("output exactly one uppercase letter from ABCD", prompt)
        self.assertTrue(prompt.endswith(IMAGE_TOKEN))

    def test_concise_reference_stage2_keeps_global_coordinates_and_two_images(self):
        sample = self.sample(self.FOUR_CHOICE_CATEGORY)
        global_question = (
            "Determine the color. Image resolution: 1024 x 683. "
            "Bounding box: [124, 288, 130, 296]."
        )
        stage1_prompt = make_stage1_prompt(global_question, 1, sample)

        prompt = make_concise_reference_stage2_prompt(stage1_prompt, 1)

        self.assertEqual(prompt.count(IMAGE_TOKEN), 2)
        self.assertIn(global_question, prompt)
        self.assertIn("The first image is a downsampled global view", prompt)
        self.assertIn("the last is a high-resolution\ncrop", prompt)
        self.assertNotIn("generate a structured reasoning answer", prompt)
        self.assertNotIn("crop's local coordinates", prompt)
        self.assertNotIn("<think>", prompt)
        self.assertIn("output exactly one uppercase letter from ABCD", prompt)
        self.assertLess(prompt.find(IMAGE_TOKEN), prompt.find(global_question))
        assistant_suffix = "<|im_end|><|im_start|>assistant\n"
        self.assertTrue(prompt.endswith(assistant_suffix))
        self.assertLess(prompt.rfind(IMAGE_TOKEN), prompt.rfind(assistant_suffix))

    def test_concise_reference_stage2_only_repositions_crop_token(self):
        sample = self.sample(self.FOUR_CHOICE_CATEGORY)
        stage1_prompt = make_stage1_prompt("question", 1, sample)
        prompt = make_concise_reference_stage2_prompt(stage1_prompt, 1)
        assistant_suffix = "<|im_end|><|im_start|>assistant\n"

        v08_layout = (
            prompt[: -len(assistant_suffix) - len(IMAGE_TOKEN)]
            + assistant_suffix
            + IMAGE_TOKEN
        )

        self.assertTrue(v08_layout.endswith(IMAGE_TOKEN))
        self.assertEqual(
            prompt.replace(IMAGE_TOKEN, ""),
            v08_layout.replace(IMAGE_TOKEN, ""),
        )

    def test_non_reference_stage2_prompt_keeps_stage1_think(self):
        stage1_prompt = "stage1 prompt"
        stage1_output = "<think>keep this reasoning</think><answer>A</answer>"

        prompt = make_stage2_prompt(stage1_prompt, stage1_output, 1)

        self.assertEqual(
            prompt,
            stage1_prompt + "<think>keep this reasoning</think>" + IMAGE_TOKEN,
        )
